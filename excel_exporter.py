"""
消防及義消子女獎學金 AI 智慧審核系統 - Excel 格式化匯出模組 (含案件編號、兩階層組織單位與獎學金類別分頁)
"""
...
from org_structure import SCHOLARSHIP_TYPES
...
def _safe_sheet_name(name: str) -> str:
    """Excel 工作表名稱限制：不可超過 31 字元，且不可包含 : \\ / ? * [ ]"""
    for ch in [":", "\\", "/", "?", "*", "[", "]"]:
        name = name.replace(ch, "")
    return name[:31] if len(name) > 31 else name


def _build_sheet(ws, records: List[Dict[str, Any]], type_label: str):
    """在指定的工作表上，寫入單一獎學金類別的完整格式化審核清冊"""
...
def export_scholarship_excel(records: List[Dict[str, Any]]) -> io.BytesIO:
    ...
    依「獎學金類別」(義消聯合總會獎助學金 / 本局津芳冰城陳慶銳先生獎學金) 分別建立獨立分頁，
    若有舊資料缺少此欄位，則另外歸入「未分類」分頁。
    """
    wb = openpyxl.Workbook()
    grouped = _group_records_by_type(records)

    for i, (type_label, type_records) in enumerate(grouped):
        ws = wb.active if i == 0 else wb.create_sheet()
        ws.title = _safe_sheet_name(type_label)
        _build_sheet(ws, type_records, type_label)
