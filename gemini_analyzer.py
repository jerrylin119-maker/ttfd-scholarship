...
2. 若成績單為五等第制或 GPA (4.0/4.3)，請換算為百分制或直接擷取百分制欄位。
   國小組成績單常只標示等第（優/甲/乙/丙/丁），沒有數字分數，此時請依下列對照換算，取該等第對應級距的代表值填入 semester_gpa（僅供資格判定使用），並在 notes 註明「學期總平均由等第換算」：
   - 優 (90分以上，不含滿分時可用 95 代表) → 95
   - 甲 (80分以上未滿90分) → 85
   - 乙 (70分以上未滿80分) → 75
   - 丙 (60分以上未滿70分) → 65
   - 丁 (未滿60分) → 55
   若成績單同時有數字分數與等第，一律以數字分數為準，不使用上述換算。
